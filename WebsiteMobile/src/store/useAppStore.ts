...
type AppState = {
  ...
  /** Last person picked via any PersonSelector, so calculator screens can default to them
   *  instead of making the user re-pick every visit. See useDefaultPerson(). */
  lastUsedPersonId: string | null;
  setLastUsedPersonId: (id: string | null) => void;
};
...
export const useAppStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      lastUsedPersonId: null,
      setLastUsedPersonId: (id) => set({ lastUsedPersonId: id }),
    }),
    {
      ...
      partialize: (state) => ({
        ...
        lastUsedPersonId: state.lastUsedPersonId,
      }),
    }
  )
);
