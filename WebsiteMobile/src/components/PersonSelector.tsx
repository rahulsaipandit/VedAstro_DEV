...
export function PersonSelector({
  ...
  return (
    <ThemedView style={styles.container}>
      {selectedPerson ? (
        // Already have someone (a remembered default or a prior pick this session) - no need for
        // the full label + big box every time; a slim "for so-and-so, change?" row is enough.
        <Pressable onPress={() => setModalVisible(true)} style={styles.compactRow} hitSlop={{ top: 8, bottom: 8, left: 0, right: 0 }}>
          <Icon name="user" size={14} color={theme.textSecondary} />
          <ThemedText type="small" themeColor="textSecondary">
            {label} for
          </ThemedText>
          <ThemedText type="smallBold" numberOfLines={1} style={styles.compactName}>
            {selectedPerson.name}
          </ThemedText>
        ...
        <>
          ...
          <Pressable
            ...
            <ThemedText>Select Person</ThemedText>
          </Pressable>
        </>
      )}
